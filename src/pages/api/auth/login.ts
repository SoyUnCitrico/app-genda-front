...
export default async function loginHandler(req : any, res : any) {
    ...
    try {
        ...
        // console.log("DATA: inlog", fetching.data)
        // if(fetching.status === 200) {
        if(fetching.data !== 'El usuario no se ha encontrado con los datos de tu busqueda'){
        ...
        else return res.status(500).json('Datos incorrectos')
